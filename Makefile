...
superuser:
	docker compose --env-file ./backend/.envs/.env.local -f docker-compose.dev.yml run --rm backend python manage.py createsuperuser

flush:
	docker compose --env-file ./backend/.envs/.env.local -f docker-compose.dev.yml run --rm backend python manage.py flush