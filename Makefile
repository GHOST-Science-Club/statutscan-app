...
superuser:
	docker compose --env-file ./backend/.envs/.env.local -f docker-compose.dev.yml run --rm app python manage.py createsuperuser

flush:
	docker compose --env-file ./backend/.envs/.env.local -f docker-compose.dev.yml run --rm app python manage.py flush